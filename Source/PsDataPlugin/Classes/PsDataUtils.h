...
constexpr uint32 crc32_impl(unsigned int crc, const char* s)
{
	return (*s == 0) ? crc ^ 0xFFFFFFFF : crc32_impl(crc32_table[static_cast<unsigned char>(crc) ^ static_cast<unsigned char>(*s)] ^ (crc >> 8), s + 1);
}

constexpr uint32 crc32(const char* s)
{
	return crc32_impl(0xFFFFFFFF, s);
}

namespace Type
{

constexpr int GetSize(const char* s, int start = 0)
{
...
struct FType
{
	...
	{
		return crc32(__FUNCTION_SIGNATURE__);
	}
};
...
struct FType<T*>
{
	...
	static const FString& Type()
	{
		static const FString Result = FString::Printf(TEXT("%s*"), *ContentType());
		return Result;
	}
...
struct FType<TArray<T>>
{
	...
	static const FString& Type()
	{
		static const FString Result = FString::Printf(TEXT("TArray<%s>"), *ContentType());
		return Result;
	}
...
struct FType<TMap<FString, T>>
{
	...
	static const FString& Type()
	{
		static const FString Result = FString::Printf(TEXT("TMap<FString, %s>"), *ContentType());
		return Result;
	}
